...
if(isset($_GET['ID_DAFTAR'])){
	...
	$cek = mysqli_query($koneksi, "SELECT * FROM daftar WHERE ID_DAFTAR='$id'") or die(mysqli_error($koneksi));
	
	//jika query menghasilkan nilai > 0 maka eksekusi script di bawah
	if(mysqli_num_rows($cek) > 0){
		//query ke database DELETE untuk menghapus data dengan kondisi id=$id
		$del = mysqli_query($koneksi, "DELETE FROM daftar WHERE ID_DAFTAR='$id'") or die(mysqli_error($koneksi));
		if($del){
