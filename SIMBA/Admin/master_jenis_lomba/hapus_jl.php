...
if(isset($_GET['ID_JENIS_LOMBA'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		$del = mysqli_query($koneksi, "DELETE FROM jenis_lomba WHERE ID_JENIS_LOMBA='$ID_JENIS_LOMBA'") or die(mysqli_error($koneksi));
		if($del){
			echo '<script>alert("Berhasil menghapus data."); document.location="jenis_lomba.php";</script>';
		}else{
